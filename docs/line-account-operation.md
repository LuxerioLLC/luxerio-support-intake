...
LINE標準のキーワード応答は完全一致のため、会社名・担当者名とキーワードを同じ文章に混ぜない。
...
POS・決済、シフト管理、補助金・助成金のフォーム案内を自動でお送りします。
...
そのため、会社名・担当者名は通常メッセージで回収し、フォーム案内だけをキーワード応答にする。
...
注意:
会社名・担当者名と `フォーム` を同じメッセージに入れると、キーワード応答が発火しない可能性が高い。
あいさつメッセージでは、会社名・担当者名を送信後、続けて `フォーム` を単独で送るよう案内する。
