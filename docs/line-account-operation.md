...
送信後、画面下部のメニューから該当するフォームをお選びください。
POS・決済、シフト管理、補助金・助成金の各フォームをご用意しています。
...
注意:
応答時間外は既存の応答メッセージが動く可能性があるため、テスト時に不要な自動返信が出ないか確認する。
不要な自動返信が出る場合は、応答時間外も手動チャットに寄せるか、応答メッセージを利用停止する。
